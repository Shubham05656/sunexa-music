    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if user and check_password_hash(user['password'],password):
                ...
                sub=cur2.fetchone(); cur2.close(); conn2.close()
...
def premium():
    ...
    cur.execute("""
        ...
        ORDER BY id DESC LIMIT 1
    """,(uid,))
    sub=cur.fetchone(); cur.close(); conn.close()
    if sub:
...
def api_activate_premium():
    ...
    try:
        ...
        cur.execute("UPDATE users SET is_premium=TRUE WHERE id=%s",(uid,))
        conn.commit()
        session['is_premium']=True
        log_user(uid,'premium_activated',f'{plan} plan Rs.{price} via {method}')
        log_admin('premium_activated',f'User {uid} activated {plan} Rs.{price}')
        cur.close(); conn.close()
...
def api_songs():
    ...
    songs=cur.fetchall(); cur.close(); conn.close()
    for s in songs:
...
def api_search():
    ...
    conn=get_db()
    cur=conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    cur.execute("SELECT * FROM songs WHERE title ILIKE %s OR artist ILIKE %s LIMIT 20",
                (f'%{q}%',f'%{q}%'))
    songs=cur.fetchall(); cur.close(); conn.close()
    for s in songs:
        if s.get('created_at'): s['created_at']=str(s['created_at'])
...
def api_create_playlist():
    ...
    conn=get_db()
    cur=conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    cur.execute("INSERT INTO playlists (user_id,name) VALUES (%s,%s) RETURNING id",(session['user_id'],name))
    pid=cur.fetchone()['id']
    conn.commit(); cur.close(); conn.close()
...
def admin_revoke_premium():
    ...
    try:
        ...
        cur=conn.cursor()
        cur.execute("UPDATE premium_subscriptions SET status='cancelled' WHERE user_id=%s AND status='active'",(uid,))
        cur.execute("UPDATE users SET is_premium=FALSE WHERE id=%s",(uid,))
        conn.commit(); cur.close(); conn.close()
